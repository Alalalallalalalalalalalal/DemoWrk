...
export default function AccountsUSMap({ data = [], onStateClick }) {
  ...
  const stateStats = useMemo(() => {
    ...
    data.forEach((item) => {
      ...
      const memberTotal = Number(item?.member_total ?? 0);
      const guestTotal = Number(item?.guest_total ?? 0);

      stats.set(stateCode, {
        total: Number.isFinite(total) ? total : 0,
        memberTotal: Number.isFinite(memberTotal) ? memberTotal : 0,
        guestTotal: Number.isFinite(guestTotal) ? guestTotal : 0,
      });
    });
  ...
  const maximumTotal = useMemo(() => {
    const totals = Array.from(stateStats.values()).map(
      (item) => item.total,
    );
  ...
  return (
    ...
    >
            ...
            geographies.map((geo) => {
              ...
              return (
                <Geography
                  key={geo.rsmKey}
                  geography={geo}
                  fill={getStateColor(stats.total, maximumTotal)}
                  ...
                  onClick={() => {
                    onStateClick?.({
                      ...
                      name: state.name,
                      total: stats.total,
                      member_total: stats.memberTotal,
                      guest_total: stats.guestTotal,
                    });
                  }}
                  onMouseLeave={() => {
      ...
      {hoveredState && (
        <div
          style={{
            ...
            minWidth: 190,
            padding: "10px 12px",
            border: "1px solid var(--dashboard-oatmeal)",
            borderRadius: 10,
            background: "var(--dashboard-palladian)",
            boxShadow: "0 6px 18px rgba(27, 38, 50, 0.14)",
            pointerEvents: "none",
            zIndex: 5,
          }}
        >
          ...
          {hoveredState.total > 0 ? (
            <>
              ...
              >
                {hoveredState.memberTotal.toLocaleString()} members ·{" "}
                {hoveredState.guestTotal.toLocaleString()} guests
              </div>

              <div
                style={{
                  marginTop: 2,
                  color: "var(--dashboard-muted, #9A8E84)",
                  fontFamily: "Inter, system-ui, sans-serif",
                  fontSize: 10,
                }}
              >
                {(
                  (hoveredState.memberTotal / hoveredState.total) *
                  100
                ).toFixed(1)}
                % members ·{" "}
                {(
                  (hoveredState.guestTotal / hoveredState.total) *
                  100
                ).toFixed(1)}
                % guests
              </div>
      ...
      >
        <LegendItem color="var(--dashboard-oatmeal)" label="No accounts" />
        <LegendItem color="var(--dashboard-flame)" label="Low" />
        <LegendItem color="#D4895A" label="Low–medium" />
        <LegendItem color="#5B8FA8" label="Medium" />
        <LegendItem color="var(--dashboard-deep-blue)" label="High" />
      </div>
      ...
      >
        Colour intensity represents total accounts
      </div>
    </div>
  );
}
    ...
    >
      <span
        style={{
          ...
          borderRadius: 2,
          background: color,
          border: "1px solid rgba(27, 38, 50, 0.12)",
        }}
      />
